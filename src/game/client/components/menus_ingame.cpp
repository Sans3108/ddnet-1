...
void CMenus::RenderInGameSans(CUIRect MainView)
{
	...
	Container.Draw(ColorRGBA(1.0f, 1.0f, 1.0f, 0.25f), IGraphics::CORNER_ALL, 10.0f);
	Container.Margin(10.0f, &Container);

	Container.HSplitTop(46.0f, &Header, &Content);
	Header.Draw(ColorRGBA(.6f, .6f, .6f, 0.25f), IGraphics::CORNER_ALL, 10.0f);
	Content.Draw(ColorRGBA(1.0f, 1.0f, 1.0f, 0.25f), IGraphics::CORNER_ALL, 10.0f);
	...
	for(int i = 0, Count = 0; i < MAX_CLIENTS; ++i)
	{
		...
		CUIRect Row = Item.m_Rect;
		if(Count % 2 == 1) Row.Draw(ColorRGBA(1.0f, 1.0f, 1.0f, 0.25f), IGraphics::CORNER_ALL, 5.0f);
		// Row.VSplitRight(s_PlayerList.ScrollbarWidthMax() - s_PlayerList.ScrollbarWidth(), &Row, nullptr); // Make room for scroll bar

		// Row.VSplitRight(64.0f, &Row, nullptr); // Space for tee, not used rn
